...
# 화면 타이틀 설정
pygame.display.set_caption("jinseok") # 게임 이름

# FPS
clock = pygame.time.Clock()
##################################################################################################################

# 1. 사용자 게임 초기화 (배경화면, 게임내 캐릭터 등 이미지, 좌표, 속도, 폰트 등등)
...
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) # 화면 가로 크기의 절반의 해당하는 크기
enemy_y_pos = (screen_height / 2) - (enemy_height / 2)  # 화면 세로 크기에 해당하는 크기
...
#  10 fps : 1초 동안에 10번 동작 >>> 1번에 10만큼 이동 해야됨
#  20 fps : 1총 동안에 20번 동작 >>> 1번에 5만큼 이동 해야됨    
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가
        ...
        if event.type == pygame.KEYDOWN: # 키보드 자판이 눌린다면
        ...
        if event.type == pygame.KEYUP: # 키보드 자판이 때진다면
...
pygame.time.delay(2000) # ms으로 나타나기 때문에 2초는 2000으로 써야한다.
